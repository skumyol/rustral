...
pub const WIKITEXT2_RAW_ZIP_URL: &str =
    "https://wikitext.smerity.com/wikitext-2-raw-v1.zip";
pub const WIKITEXT2_RAW_ZIP_SHA256: &str =
...
fn extract_three_splits(
    zip_path: &PathBuf,
    out_dir: &PathBuf,
) -> Result<WikiText2Splits, FetchError> {
    ...
    if !status.success() {
        return Err(FetchError::Http(
            "`unzip` exited with non-zero status; install unzip or pre-stage the splits"
                .to_string(),
        ));
    }
    ...
    let base = candidates
        .iter()
        .map(|d| out_dir.join(d))
        .find(|p| p.join("wiki.train.raw").exists())
        .ok_or_else(|| {
            FetchError::Http(
