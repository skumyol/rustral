//! Metal workload runner (Phase 2 P3 I3).
//!
//! Mirrors `candle_workloads` (matmul / attention / conv2d) on `Device::Metal` (Apple
//! Silicon / macOS). Synchronises around every timed call so we measure device wall time,
//! not host queue submission.
//!
//! Gated by `--features metal` (see `crates/bench/Cargo.toml` `required-features =
//! ["metal"]`).
//!
//! Usage (Apple Silicon host):
//!
//! ```bash
...
const BACKEND: &str = "candle-metal";
const DEVICE: &str = "metal:0";
...
fn main() {
    ...
    let device = Device::new_metal(0).expect(
        "rustral_workloads_metal: failed to acquire Metal device 0; ensure macOS host and \
