...
use candle_nn::{Module, VarMap, VarBuilder, Linear, LayerNorm};

const BACKEND: &str = "candle-cpu";

struct CandleTransformerLayer {
    self_attn: (Linear, Linear, Linear, Linear), // q, k, v, out
    ln1: LayerNorm,
    ln2: LayerNorm,
    linear1: Linear,
    linear2: Linear,
...
fn main() {
    ...
    let runs = time_runs(
        || {
            let _doc_text = "The model learns quickly.";
    ...
    samples.push(Sample::cpu_f32(
        "nlp.full_pipeline",
        BACKEND,
        vec![("d_model".into(), "128".into()), ("seq_len".into(), "5".into())],
        runs,
    ));

    print!("{}", samples_to_json("candle-nlp", &samples));
}
