...
use rustral_core::{Backend, OperationProfiler, TensorPool};
...
fn test_operation_profiler() {
    ...
    profiler.record_operation_internal("matmul", std::time::Duration::from_millis(10));
    profiler.record_operation_internal("matmul", std::time::Duration::from_millis(12));
    profiler.record_operation_internal("relu", std::time::Duration::from_millis(1));
...
fn test_profiler_regression_detection() {
    ...
    baseline.record_operation_internal("test_op", std::time::Duration::from_millis(100));
    
    let mut current = OperationProfiler::new();
    current.record_operation_internal("test_op", std::time::Duration::from_millis(150));
    ...
    current2.record_operation_internal("test_op", std::time::Duration::from_millis(110));
