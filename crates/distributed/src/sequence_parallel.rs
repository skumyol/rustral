    ...
    B::Tensor: AsRef<[f32]>,
{
    ...
    if seq_len % num_shards != 0 {
        return Err(CoreError::Shape(
            format!("Sequence length {} must be divisible by num_shards {}", seq_len, num_shards)
        ));
    }
...
pub fn gather_sequence<B: Backend>(
    shards: &[B::Tensor],
    ops: &dyn TensorOps<B>,
) -> Result<B::Tensor>
where
    B::Tensor: AsRef<[f32]>,
{
    ...
    let total_seq: usize = shards.iter()
        .map(|s| ops.shape(s)[1])
        .sum();
    ...
    B::Tensor: Clone + AsRef<[f32]> + TensorShape,
{
    ...
    pub fn compute(
        ...
        for step in 0..num_devices {
            ...
            let attention_chunk = self.compute_attention_chunk(
                local_q, &k_block, &v_block, ops
            )?;
            ...
            for h in 0..num_heads {
                for i in 0..q_len {
                    for j in 0..k_len {
                        ...
                        let q_idx = b * q_len * num_heads * head_dim + i * num_heads * head_dim + h * head_dim;
                        let k_idx = b * k_len * num_heads * head_dim + j * num_heads * head_dim + h * head_dim;
                        ...
                        let score_idx = b * q_len * k_len * num_heads + i * k_len * num_heads + j * num_heads + h;
            ...
            for h in 0..num_heads {
                for i in 0..q_len {
                    for d in 0..head_dim {
                        let mut val = 0.0;
                        for j in 0..k_len {
                            let w_idx = b * q_len * k_len * num_heads + i * k_len * num_heads + j * num_heads + h;
                            let v_idx = b * k_len * num_heads * head_dim + j * num_heads * head_dim + h * head_dim + d;
                            val += weights[w_idx] * v_data[v_idx];
                        }
                        let out_idx = b * q_len * num_heads * head_dim + i * num_heads * head_dim + h * head_dim + d;
                        output[out_idx] = val;
                    }
                }
            }
    ...
    fn compute_gathered(
        ...
        self.compute_attention_chunk(local_q, local_k, local_v, _ops)
            .and_then(|data| {
...
pub fn compute_sequence_sharding(
    total_seq_len: usize,
    num_devices: usize,
) -> Result<(usize, usize)> {
    if total_seq_len % num_devices != 0 {
        return Err(CoreError::InvalidArgument(
            format!("Sequence length {} not divisible by {}", total_seq_len, num_devices)
        ));
    }
...
mod tests {
    ...
    fn test_ring_attention_config() {
        let config = SequenceParallelConfig::new(4)
            .with_block_size(512)
    ...
    fn test_ring_attention_compute() {
        let backend = CpuBackend::default();
        let config = SequenceParallelConfig::new(1)
            .with_block_size(4)
            .with_ring_attention(true);
