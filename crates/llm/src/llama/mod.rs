...
use rustral_nn::{LlamaDecodeCache, LlamaDecoderConfig};
...
impl HfLlamaConfig {
    pub fn from_json_file(path: impl AsRef<Path>) -> Result<Self, LlmError> {
        ...
        let cfg: Self =
            serde_json::from_slice(&bytes).with_context(|| "parse Llama config json".to_string())?;
        Ok(cfg)
    }

    pub fn validate_supported(&self) -> Result<(), LlmError> {
        let kv = self.num_key_value_heads.unwrap_or(self.num_attention_heads);
        if self.num_attention_heads % kv != 0 {
            return Err(LlmError::InvalidArg(format!(
                "num_attention_heads ({}) must be divisible by num_key_value_heads ({kv})",
...
impl LlamaCausalLm {
    ...
    pub fn from_hf_meta(
        ...
        meta: &MetaStateDict,
        seed: u64,
    ) -> Result<(Self, LlamaWeightLoadReport), LlmError> {
    ...
    fn greedy_steps(
        ...
        let logits = self
            .model
            .forward_prompt_cache(input_ids.clone(), ctx, &mut cache)
            .map_err(|e| LlmError::InvalidArg(format!("Llama KV prefill: {e:?}")))?;
        let ops = self.backend.ops();
        let mut next = argmax_last_timestep(&logits, ops)?;
        input_ids.push(next);
        for _ in 1..steps {
            let logits = self
                .model
                .forward_token_cache(next, ctx, &mut cache)
                .map_err(|e| LlmError::InvalidArg(format!("Llama KV decode step: {e:?}")))?;
            next = argmax_single_token_logits(&logits, ops)?;
            input_ids.push(next);
        }
        Ok(input_ids)
    }
    ...
    pub fn generate_greedy_timed(
        ...
        if max_new_tokens == 0 {
            return Ok((
                input_ids,
                crate::gpt2::GreedyDecodeTiming { first_token_ms: 0.0, decode_wall_ms: 0.0 },
            ));
        }
        ...
        let logits = self
            .model
            .forward_prompt_cache(input_ids.clone(), &mut ctx, &mut cache)
            .map_err(|e| LlmError::InvalidArg(format!("Llama KV prefill: {e:?}")))?;
        let ops = self.backend.ops();
        let mut next = argmax_last_timestep(&logits, ops)?;
        ...
        for _ in 1..max_new_tokens {
            let logits = self
                .model
                .forward_token_cache(next, &mut ctx, &mut cache)
                .map_err(|e| LlmError::InvalidArg(format!("Llama KV decode step: {e:?}")))?;
            next = argmax_single_token_logits(&logits, ops)?;
            input_ids.push(next);
        }
        let decode_wall_ms = decode_start.elapsed().as_secs_f64() * 1000.0;
        Ok((input_ids, crate::gpt2::GreedyDecodeTiming { first_token_ms, decode_wall_ms }))
    }
...
fn argmax_last_timestep(
    logits: &<CpuBackend as Backend>::Tensor,
    ops: &dyn TensorOps<CpuBackend>,
) -> Result<usize, LlmError> {
    let shape = ops.shape(logits);
    if shape.len() != 3 {
        return Err(LlmError::InvalidArg("expected [batch, seq, vocab] logits".into()));
    }
    let vocab = shape[2];
    let seq = shape[1];
    let flat = ops.tensor_to_vec(logits).map_err(|e| LlmError::InvalidArg(format!("{e:?}")))?;
    let offset = (seq - 1) * vocab;
    let row = &flat[offset..offset + vocab];
    row.iter()
        .enumerate()
        .max_by(|(_, a), (_, b)| a.partial_cmp(b).unwrap())
        .map(|(i, _)| i)
        .ok_or_else(|| LlmError::InvalidArg("empty logits row".into()))
}

fn argmax_single_token_logits(
    logits: &<CpuBackend as Backend>::Tensor,
    ops: &dyn TensorOps<CpuBackend>,
) -> Result<usize, LlmError> {
    let shape = ops.shape(logits);
    if shape.len() != 3 || shape[1] != 1 {
        return Err(LlmError::InvalidArg("expected [batch, 1, vocab] logits".into()));
    }
    let vocab = shape[2];
    let flat = ops.tensor_to_vec(logits).map_err(|e| LlmError::InvalidArg(format!("{e:?}")))?;
    let row = &flat[..vocab];
