...
fn main() -> Result<()> {
    ...
    fn create_model<B: Backend>(backend: &B) -> Result<(LinearReLU<B>, LinearGELU<B>, Linear<B>)> {
        let config_relu = LinearConfig { in_dim: 128, out_dim: 256, bias: true };

        let config_gelu = LinearConfig { in_dim: 256, out_dim: 128, bias: true };

        let config_linear = LinearConfig { in_dim: 128, out_dim: 64, bias: true };
    ...
    let cpu_input = cpu_backend.tensor_from_vec(vec![1.0f32; 128], &[1, 128])?;
